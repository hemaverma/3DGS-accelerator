...
impl ColmapRunner {
    ...
    pub fn new() -> Self {
        // Use COLMAP_BIN env var if set, otherwise use 'colmap' from PATH
        let colmap_bin = std::env::var("COLMAP_BIN").unwrap_or_else(|_| "colmap".to_string());

        ColmapRunner { colmap_bin }
    }
    ...
    pub async fn extract_features(
        ...
        tokio::task::spawn_blocking(move || {
            debug!(
                "Running feature extraction: {} images, camera model {}",
                image_dir.display(),
                camera_model
            );
